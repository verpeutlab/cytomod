...
def partialCorrNormalize(cyDf, cyVars=None, compCommVars=None, meanVar=None, returnModels=True):
    ...
    def _meanCorrModel(colVec):
        if colVec.isnull().all():
            ...
            model = sm.GLM(endog=colVec, exog=sm.add_constant(muVec), missing='drop') #compute GLM. x=1s+muVec, y=colVec
            try:
                result = model.fit()
    ...
    muVec = standardizedMean(cyDf, vars=compCommVars) # compute mean after standardizing
    
    models = cyDf.loc[:, cyVars].apply(_meanCorrModel, axis=0) # compute GLM for each cy. x=cy, y=muVec
...
def enforceSensitivity(cyDf, sensitivityS, truncateLevels=True, inplace=True):
    ...
    for col in cyDf.columns:
        ...
        if any(gtPresent):
            mx = cyDf.loc[gtPresent, col].map(partial(convertLevel, 0, None)).max()
        else:
            mx = None
